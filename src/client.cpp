...
public:
    DBMSClient(const std::string& server_ip = "127.0.0.1", int port = 8080) 
        : sock_(INVALID_SOCK), server_ip_(server_ip), port_(port) {
        #ifdef _WIN32
    ...
    bool connect() {
        ...
        if (::connect(sock_, (struct sockaddr*)&server_addr, sizeof(server_addr)) == SOCK_ERR) {
    ...
    std::string send_query(const std::string& query) {
        ...
        send(sock_, query_with_newline.c_str(), (int)query_with_newline.size(), 0);
